"""
Pipeline stage 6: render documents/HONESTY_REPORT.md from the raw eval
data pipeline_05 produced. Metrics, counts, and the ELL bias-probe
numbers are filled in mechanically from processed/eval_report.json --
only the "hypothesis for why" prose on the three confidently-wrong
examples is written by hand (inspecting the actual failures), inserted
via the HYPOTHESES dict below.
"""

import json

from config import HONESTY_REPORT_PATH, PROCESSED_DATA_DIR

EVAL_REPORT_PATH = PROCESSED_DATA_DIR / "eval_report.json"

# Filled in by hand after inspecting the actual confidently-wrong
# examples pipeline_05 surfaces (see the printed doc_id / bucket / text
# preview). Keyed by doc_id.
HYPOTHESES: dict[str, str] = {
    "daigt_001681": (
        "A false negative on an essay written with heavy deliberate phonetic misspelling (\"shuld\", \"da rite\", "
        "\"teechers\", \"sposed\", \"wut theyre doing\") -- whichever DAIGT generator produced this was evidently "
        "prompted to imitate a young or careless student's spelling. Our hypothesis: this breaks the same "
        "assumption as the non-native-English case (see the fairness probe below) from the opposite direction -- "
        "a model imitating non-standard orthography produces token sequences that are genuinely unusual/hard to "
        "predict for GPT-2, which our signal layer reads as human-typical unpredictability. Non-standard spelling "
        "of any origin, real or imitated, seems to be a blind spot."
    ),
    "daigt_004069": (
        "This is a false negative: true AI, predicted human with high confidence. The essay is written in a "
        "convincingly casual first-person voice (\"I have experience volunteering at school...\") describing "
        "concrete, specific personal history. Our signal and feature layers were built around a real but "
        "imperfect correlation -- AI text is *usually* more uniform/predictable and more formal -- and this "
        "essay breaks that correlation: whichever model in the DAIGT mix generated it produced something with "
        "human-like burstiness and informal register. This is precisely the kind of essay the product is "
        "weakest against: AI output deliberately or incidentally imitating an informal personal register."
    ),
    "daigt_001744": (
        "Another false negative, on a source-based analytical prompt (\"The Enigma of the Face on Mars\", citing "
        "\"the article\"). Our hypothesis: source-based/evidence-citing prompts constrain both human and AI "
        "writers toward similar structure (topic sentence, numbered evidence, citation phrasing), which "
        "compresses the stylistic gap our features look for -- burstiness and lexical diversity in particular "
        "are driven partly by the *task*, not just by who/what wrote it. This suggests the detector is weaker on "
        "structured/source-based writing tasks than on open-ended personal narrative, which matters since real "
        "admissions essays are closer to the latter -- but any supplemental essay asking students to respond to "
        "provided material would land in this weaker zone."
    ),
    "daigt_001384": (
        "A false negative on an unusually chaotic example: heavy slang (\"gotta\", \"Like, I know\") mixed with "
        "garbled emoji-name artifacts (\"cozy faceblush* speedometer emojinervous smile*\"), apparently a model "
        "failure mode from whichever generator produced it. Our hypothesis: the garbled tokens are genuinely "
        "unpredictable to GPT-2 (high per-token surprisal), which is exactly the signal we treat as evidence of "
        "human writing -- the feature is not wrong about what it measures, but 'unpredictable because broken' "
        "and 'unpredictable because a human wrote it' look identical to a model that only sees token "
        "probabilities. A real human essay full of typos or unusual formatting would likely trigger the same "
...
def render_metrics_table(name: str, m: dict) -> str:
    cm = m["confusion_matrix"]
    return f"""**{name}** (n={m['n']} test sentences, {m['n_positive']} true-AI)
...
def render_wrong_examples(examples: list[dict]) -> str:
    if not examples:
        return "_No confidently-wrong pure-label documents found in the test split._\n"
    blocks = []
    for i, ex in enumerate(examples, 1):
        hyp = HYPOTHESES.get(ex["doc_id"], "_(hypothesis not yet filled in -- see backend/pipeline_06_write_honesty_report.py)_")
        blocks.append(f"""### {i}. `{ex['doc_id']}` (bucket: {ex['bucket']})

- True label: **{'AI' if ex['true_label'] else 'human'}** — Predicted: **{'AI' if ex['pred_label'] else 'human'}** (mean sentence-level P(AI) = {ex['mean_prob']:.3f})
- {ex['n_sentences']} sentences in this document
...
def render_bias_probe(probe: dict | None) -> str:
    if probe is None:
        return "_ELL fairness probe not run (backend/data/processed/ell_sentence_features.parquet missing)._\n"
    gap = probe["gap_percentage_points"]
    direction = "OVER-flags" if gap > 0 else "does NOT over-flag (if anything, under-flags)"
    return f"""We evaluated the trained detector on **{probe['ell_n_sentences']} sentences from {probe['ell_n_essays']} essays**
...
|---|---|---|
| Non-native English (ELL) | {probe['ell_n_sentences']} | {fmt_pct(probe['ell_false_positive_rate'])} |
| Native English (test split) | {probe['native_human_n_sentences']} | {fmt_pct(probe['native_human_false_positive_rate'])} |

**Finding: the detector {direction} non-native English writing, by {abs(gap):.1f} percentage points.**
...
Train set: {report['n_train_docs']} documents / {report['n_train_sentences']} sentences.
Test set: {report['n_test_docs']} documents / {report['n_test_sentences']} sentences.
...
held-out F1 from 0.932 to {report['logreg_metrics']['f1']:.3f}, which we consider the right trade: a model that
...
{render_metrics_table("Logistic regression (shipped model)", report['logreg_metrics'])}

{render_metrics_table("Gradient-boosted trees (comparison only, not shipped)", report['gbt_metrics'])}

Logistic regression ships in the live app instead of the (here, {"higher" if report['gbt_metrics']['f1'] > report['logreg_metrics']['f1'] else "comparable-or-lower"}-F1)
...
{render_wrong_examples(report['confidently_wrong_examples'])}

## Does the detector over-flag non-native English writing?

{render_bias_probe(report['ell_bias_probe'])}
...
- **Real false-positive rate on native-English human writing is {fmt_pct(report['ell_bias_probe']['native_human_false_positive_rate']) if report.get('ell_bias_probe') else 'notable'}** at the
...
"""
    HONESTY_REPORT_PATH.write_text(md, encoding="utf-8")
    print(f"Wrote {HONESTY_REPORT_PATH}")
